import { TreeNode, recurse } from "../utils";
import { TreeDataConfig } from "./type";

export default class VisualTreeData<D, K> extends Array {
	dataKey: PropertyKey = "id";
	dataMap = {};

	protected shrinkSet = new Set<PropertyKey>();
	protected hiddenSet = new Set<PropertyKey>();
	protected searchSet = new Set<PropertyKey>();
	...
	constructor(data: TreeNode<D, "children">[], key = "id", opt: Partial<TreeDataConfig> = {}) {
		super();
		this.dataKey = key;
	...
	slice(b?: number, e?: number) {
		...
		const end = typeof e !== "undefined" ? e : len;
		const start = b || 0;
		const begin = start >= 0 ? start : Math.max(0, len + start);
		const upTo = end < 0 ? (typeof end == "number" ? Math.min(end, len) : len) : len + end;
		const size = upTo - begin;
		if (size > 0) {
			const cloned = new Array(size);
			for (let i = 0; i < size; i++) cloned[i] = this[start + i];
			return cloned;
		}
		return [];
	}
	...
	deleteNodeFromParent(item) {
		if (item._parentNode) {
			item._parentNode.children = item._parentNode.children.filter(
				(child) => child[this.dataKey] !== item[this.dataKey]
			);
		}
	}
	...
	sortTree(fn) {
		...
		let newIndex = 0;
		const topLevelNodes = Object.values(this.dataMap).filter(
			(node) => node._parentNode === null
		);
	...
	filterTree(fn) {
		...
		const hideNodeWithParent = (node, fn) => {
			// isSaveParent 为 true
		...
		const hideNodeWithoutParent = (node, fn) => {
			// isSaveParent 为 false
			if (resultMap[node[this.dataKey]] !== void 0) return resultMap[node[this.dataKey]];
			// 如果是叶子节点
			...
			node.children.forEach(
				(child) => hideNodeWithoutParent(child, fn) && (retainCurrent = true)
			);
			return hideNode(node, retainCurrent);
		};
		Object.values(this.dataMap).forEach((node) =>
			isSaveParent ? hideNodeWithParent(node, fn) : hideNodeWithoutParent(node, fn)
		);
	...
	updateSearchAndFilterResult() {
		// 已展开的数据里
		let newArr = Object.values(this.dataMap).filter(
			(node) => !this.shrinkSet.has(node[this.dataKey])
		);
